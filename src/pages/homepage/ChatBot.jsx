...
function Chatbot() {
  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await fetch("https://lokajatim.org/chatbot", {
        method: "POST",
        headers: {
  ...
  return (
    <div className="fixed bottom-4 right-4 w-[375px] h-[600px] flex flex-col bg-white shadow-lg rounded-xl">
      {/* Header */}
      <div className="w-full flex items-center justify-between bg-[#8B5428] text-white px-4 py-3 shadow-lg rounded-t-xl">
      <h1 className="text-[20px] font-roboto font-[400] leading-[24px] flex items-center">
        ...
        <button className="w-[147px] h-[36px] rounded-[16px] border border-[#FFFFFF] bg-[#8B5428] text-white text-[16px] font-roboto font-[400]">
          Beri Feedback
        </button>
      ...
      {/* Chat Input */}
      <div className="w-full flex items-center bg-[#8B5428] px-3 py-3">
        <input
          type="text"
          value={userInput}
